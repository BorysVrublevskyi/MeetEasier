...
const Time = ({room, details}) => (
  <div id="single-room__meeting-time">
    { details.appointmentExists &&
      new Date(parseInt(room.Appointments[0].Start, 10)).toLocaleTimeString([], {weekday: 'short', hour: '2-digit', minute: '2-digit', 'hour12': false})
      + ' - ' + 
      new Date(parseInt(room.Appointments[0].End, 10)).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit', 'hour12': false})
    }
  </div>
);
...
const RoomStatusBlock = ({ config, details, room, togglePopup, showPopup }) => (
  <div className={room.Busy ? 'columns large-7 left-col busy' : 'columns large-7 left-col open'}>
  <table id="single-room__logo-name-table">
    <tr><td id="single-room__org-logo-td">
    <div id="single-room__org-logo">
      <img src="../img/logo.png" alt="Logo" id="single-room__org-logo__img" />
    </div>
    </td>
    <td>
